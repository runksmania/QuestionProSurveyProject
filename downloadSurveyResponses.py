...
from getResponses import getResponses
from importantColFinder import getColLoc
from excelManager import exportData
...
def checkNeccessaryFiles():
    ...
    if not os.path.exists('spreadsheet_ids.csv'):
        fileMissing = True

        with open('spreadsheet_ids.csv', 'w') as outputFile:
    ...
    if not os.path.exists('answer_replacements.csv'):
        fileMissing = True

        with open('answer_replacements.csv', 'w') as outputFile:
            print('spreadsheet id,column name,"answer list","replacement list",County Question (1 if yes, 0 if no)',
                  file=outputFile)

            print(
                'Please edit the answer_replacements.csv file and add the answers you want to be replaced.')
            print('When finished re-run this program.\n')
...
def responseManager(creds):
    ...
    csvData = process_csv('answer_replacements.csv')
    answerDict = {}
